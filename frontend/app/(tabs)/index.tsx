...
// Reload inbox after short delay to persist new conversations
setTimeout(() => {
  if (user?._id) {
    chatApi.getConversations(user._id)
      .then((convs) => setInbox(convs))
                ...
                return (
                  <TouchableOpacity key={chat.partnerId} style={[styles.chatItem, hasUnread && { backgroundColor: 'rgba(193, 123, 58, 0.1)', borderColor: THEME.primary, borderWidth: 1.5 }]} onPress={() => openChat(chat)}>
                    ...
                    <View style={{ flex: 1, marginLeft: 15 }}>
                      <Text style={{ fontWeight: '700', color: hasUnread ? THEME.primary : THEME.textMain, fontSize: 16 }}>{chat.partnerName}</Text>
                      <Text style={{ color: THEME.textMuted, fontSize: 13 }} numberOfLines={1}>{chat.lastMessage}</Text>
                    </View>
...
export default function App() {
  ...
  error: btError,
  isLoading: btLoading,
  isScanning: btIsScanning,
  fetchPairedDevices: btFetchDevices,
  startScan: btStartScan,
  stopScan: btStopScan,
    ...
    <ScrollView style={{ flex: 1, paddingHorizontal: 20 }} contentContainerStyle={{ paddingBottom: 40 }}>
      ...
      {Platform.OS === 'android' && (
        <TouchableOpacity
          style={[
            btStyles.pairNowBtn,
            { backgroundColor: btIsScanning ? THEME.danger : THEME.secondary, marginTop: -8 },
            (btLoading || btConnectState === 'connecting') && { opacity: 0.6 },
          ]}
          onPress={btIsScanning ? btStopScan : btStartScan}
          disabled={btLoading || btConnectState === 'connecting'}
        >
          <Text style={btStyles.pairNowText}>
            {btIsScanning ? 'Stop Scanning' : 'Scan for New Devices'}
          </Text>
        </TouchableOpacity>
      )}

      <Text style={btStyles.sectionLabel}>
        {btScannedDevices.length > 0 ? 'ALL DEVICES' : 'PAIRED DEVICES'}
      </Text>

      {!btLoading && btDevices.length === 0 && btScannedDevices.length === 0 && (
      ...
      {[
        ...btDevices,
        ...btScannedDevices.filter(s => !btDevices.find(p => p.id === s.id)),
      ].map((device) => {
